...
describe('Perm Check', () => {
  ...
  describe('Array of primimitives', () => {
    it("_allow - submitted 'arr', perm 'arr' true", () => {
      const { perm, data } = fxt.arr1;
      const ret = checkFieldPerm(perm, data);
      expect(ret).toBe(undefined);
    });

    it("REJECT - submitted 'arr', perm 'arr' false", () => {
      const { perm, data } = fxt.arr2;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted 'arr', perm 'arr' not exist, wildcard not exist", () => {
      const { perm, data } = fxt.arr3;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted 'arr', perm 'arr' not exist, wildcard false", () => {
      const { perm, data } = fxt.arr4;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("_allow - submitted 'arr', perm 'arr' not exist, wildcard true", () => {
      const { perm, data } = fxt.arr5;
      const ret = checkFieldPerm(perm, data);
      expect(ret).toBe(undefined);
    });
  });

  describe('Array draining', () => {
    it("REJECT - submitted empty 'arr', perm 'arr' true", () => {
      const { perm, data } = fxt.arr1drain;
      const error = new Error("You are not allowed to drain array field 'arr'.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted empty 'arr', perm 'arr' false", () => {
      const { perm, data } = fxt.arr2drain;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted empty 'arr', perm 'arr' not exist, wildcard not exist", () => {
      const { perm, data } = fxt.arr3drain;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted empty 'arr', perm 'arr' not exist, wildcard false", () => {
      const { perm, data } = fxt.arr4drain;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("_allow - submitted empty 'arr', perm 'arr' not exist, wildcard true", () => {
      const { perm, data } = fxt.arr5drain;
      const error = new Error("You are not allowed to drain array field 'arr'.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });
  });
  ...
  describe('Array of objects', () => {
    it("_allow - submitted 'arr[].foo', perm 'arr[].foo' true", () => {
      const { perm, data } = fxt.arrObj1;
      const ret = checkFieldPerm(perm, data);
      expect(ret).toBe(undefined);
    });

    it("REJECT - submitted 'arr[].foo', perm 'arr[].foo' false", () => {
      const { perm, data } = fxt.arrObj2;
      const error = new Error("You are not allowed to submit 'arr[0].foo' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted 'arr[].foo', perm 'arr[].foo' not exist, wildcard not exist", () => {
      const { perm, data } = fxt.arrObj3;
      const error = new Error("You are not allowed to submit 'arr[0].foo' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted 'arr[].foo', perm 'arr[].foo' not exist, wildcard false", () => {
      const { perm, data } = fxt.arrObj4;
      const error = new Error("You are not allowed to submit 'arr[0].foo' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("_allow - submitted 'arr[].foo', perm 'arr[].foo' not exist, wildcard true", () => {
      const { perm, data } = fxt.arrObj5;
      const ret = checkFieldPerm(perm, data);
      expect(ret).toBe(undefined);
    });

    it("REJECT - submitted 'arr[].foo', perm 'arr[]' empty", () => {
      const { perm, data } = fxt.arrObj6;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted 'arr[].foo', perm 'arr' not exist, wildcard not exist", () => {
      const { perm, data } = fxt.arrObj7;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("REJECT - submitted 'arr[].foo', perm 'arr' not exist, wildcard false", () => {
      const { perm, data } = fxt.arrObj8;
      const error = new Error("You are not allowed to submit 'arr' field.");
      expect(() => checkFieldPerm(perm, data)).toThrow(error);
    });

    it("_allow - submitted 'arr[].foo', perm 'arr' not exist, wildcard true", () => {
      const { perm, data } = fxt.arrObj9;
      const ret = checkFieldPerm(perm, data);
      expect(ret).toBe(undefined);
    });
  });
